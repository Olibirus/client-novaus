    ...
    <section id="realisations" className="relative bg-ink py-28 lg:py-40 border-t border-line">
      <div className="max-w-8xl mx-auto px-6 lg:px-12">
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
            {projects.filter(p => p.category === "novaus").map((p) => (
              <button
                key={p.src}
                onClick={() => setOpen(p)}
      ...
      {open && (
        ...
        >
          <button
            onClick={() => setOpen(null)}
            className="absolute top-6 right-6 lg:top-10 lg:right-10 text-cream hover:text-gold text-3xl font-light"
            aria-label="Fermer"
          >
            ×
          </button>
          ...
          >
            ...
            <div className="mt-6 flex items-baseline justify-between border-t border-line pt-4">
              <div>
                <p className="font-display text-2xl text-cream font-light">{open.title}</p>
                <p className="text-sm text-cream/50 mt-1">{open.meta}</p>
              </div>
